...
bool BasePlatform::InitializeLogFile(void) {
  ...
  mlog_file.Append(_T("Logs/"));  // so, on OS X, its.log ends up in
  ...
  if (::wxFileExists(mlog_file)) {
    if (wxFileName::GetSize(mlog_file) > 1000000) {
      ...
      //  established.
      large_log_message = (_T("Old log will be moved to its.log.log"));
...
wxString AbstractPlatform::GetPluginDataPath() {
  ...
  if (isFlatpacked()) {
    ...
    dirs +=
        "~/Library/Application Support/ITS/Contents/SharedSupport/plugins";
  }
#endif
