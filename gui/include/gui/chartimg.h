...
typedef enum PaletteDir { PaletteFwd, PaletteRev } _PaletteDir;
...
public:
  ...
  virtual int latlong_to_pix_vp(double lat, double lon, double &pixx,
                                double &pixy, ViewPort &vp);
  virtual int vp_pix_to_latlong(ViewPort &vp, double pixx, double pixy,
...
protected:
  ...
  virtual bool GetAndScaleData(unsigned char *ppn, size_t data_size,
                               wxRect &source, int source_stride, wxRect &dest,
                               int dest_stride, double scale_factor,
  ...
  virtual int BSBGetScanline(unsigned char *pLineBuf, int y, int xs, int xl,
                             int sub_samp);
...
// ----------------------------------------------------------------------------
// ChartPlugInWrapper
//    This class is a wrapper/interface to PlugIn charts(PlugInChartBase) as
//    defined in ocpn_plugin.h
// ----------------------------------------------------------------------------
...
public:
  ...
  //    The following set of methods apply to BSB (i.e. Raster) type PlugIn
  //    charts only and need not be implemented if the ChartFamily is not
  //    CHART_FAMILY_RASTER
  virtual void ComputeSourceRectangle(const ViewPort &vp, wxRect *pSourceRect);
  virtual double GetRasterScaleFactor(const ViewPort &vp);
  virtual bool GetChartBits(wxRect &source, unsigned char *pPix, int sub_samp);
  ...
  virtual bool RenderRegionViewOnGLTextOnly(const wxGLContext &glc,
                                            const ViewPort &VPoint,
                                            const OCPNRegion &RectRegion);
