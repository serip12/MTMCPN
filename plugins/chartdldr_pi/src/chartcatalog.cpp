...
bool ChartCatalog::LoadFromFile(wxString path, bool headerOnly) {
  ...
  date_valid = dt_valid;
  title = _("Catalog is not valid.");  // Invalidate the title in case we read a
                                       // bad file
...
wxDateTime ChartCatalog::GetReleaseDate() {
  if (!dt_valid.IsValid()) {
    // date-time was invalid so we will create it from time_created and
    // date_created If they are not valid then we return an invalid date for
    // debugging purposes
  ...
  for (pugi::xml_node element = xmldata.first_child(); element;
    ...
    if (!strcmp(element.name(), "title")) {
      ...
      for (pugi::xml_node subElement = element.first_child(); subElement;
        ...
        coast_guard_districts->Add(
            wxString::FromUTF8(subElement.first_child().value()));
      }
...
RasterChart::RasterChart(pugi::xml_node &xmldata) : Chart(xmldata) {
  ...
  for (pugi::xml_node element = xmldata.first_child(); element;
    ...
    if (!strcmp(element.name(), "number")) {
      ...
      source_edition =
          wxAtoi(wxString::FromUTF8(element.first_child().value()));
